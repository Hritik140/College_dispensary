...
const NearByHospital = (props) => {
    ...
    const fetchData = async () => {
        props.showLoader();
        await axios.get('https://college-dispensary-pro.onrender.com/api/hospital/get').then(resp => {
    ...
    const handleDelete=async(id)=>{
        props.showLoader();
        await axios.delete(`https://college-dispensary-pro.onrender.com/api/hospital/delete/${id}`,{withCredentials:true}).then(resp=>{
            filterOutData(id)
        }).catch(err => {
            toast.error(err?.response?.data?.error)
