...
const ForgotModal = (props) => {
    ...
    const sendOTPToMail = async()=>{
        ...
        props.showLoader()
        await axios.post('https://college-dispensary-pro.onrender.com/api/auth/send-otp',{email:inputField.email}).then((response)=>{
    ...
    const checkOtp = async()=>{
        ...
        await axios.post('https://college-dispensary-pro.onrender.com/api/auth/verify-otp',{email:inputField.email,otp:inputField.otp}).then((response)=>{
            setStep(3);
    ...
    const resetPassword = async()=>{
        ...
        await axios.post('https://college-dispensary-pro.onrender.com/api/auth/reset-password',{email:inputField.email,newPassword:inputField.newPassword}).then((response)=>{
            alert(response.data.message)
            props.closeModal()
        }).catch(err => {
            console.log(err)
